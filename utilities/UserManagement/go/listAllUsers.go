...
// listAllUsers calls the getusers API to iterate through the users using bookmarks
// See https://docs.verify.ibm.com/verify/reference/getusers
func listAllUsers(configInfo *ConfigInfo) (err error) {
	usersReturned := 0
	err, userList := getNextUserList(configInfo, "", configInfo.userAttribute)
	if err != nil {
		fmt.Printf("Error %s from getNextUserList\n", err)
	}
	for len(userList.Resources) > 0 {
		usersReturned += len(userList.Resources)
		fmt.Printf("Got %d users back\n", len(userList.Resources))
		err, userId := printUserList(configInfo, userList)
		if err != nil {
			fmt.Printf("Error %s from printUserList\n", err)
		}
		nextUserId := userId + "0"
		err, userList = getNextUserList(configInfo, nextUserId, configInfo.userAttribute)
		if err != nil {
			fmt.Printf("Error %s from getNextUserList\n", err)
		}
	}
...
func getNextUserList(configInfo *ConfigInfo, userStartingValue, attributeList string) (err error, userList UserList) {
	...
	nextPage := ""
	if userStartingValue != "" {
		nextPage := "&nextPage=" + userStartingValue
	}
	attributes := ""
	if attributeList != "" {
	...
	completeURL := "https://" + configInfo.tenantHostname + userEndpoint + useBookmark + nextPage + attributes + countLimit
